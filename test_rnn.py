...
DEFAULT_CONFIG = {
    ...
    "physics_normal": {  
        "box2d_gravity": {"mean": -10.0, "std": 2.0, "clip": [-20.0, -2.0]},  # 평균 -10, 표준편차 2, 클리핑 범위 -20 ~ -2
        "wind_power": {"mean": 10.0, "std": 3.0, "clip": [0.0, 25.0]},  # 평균 10 표준편차 3, 클리핑 범위 0 ~ 25
        "turbulence_power": {"mean": 1.0, "std": 0.5, "clip": [0.0, 3.0]} # 평균 1 표준편차 0.5, 클리핑 범위 0 ~ 3
    },
...
class RecurrentActorCritic(nn.Module):
    ...
    def act(self, state, hidden):
        st = torch.FloatTensor(np.array(state)).to(self.device).unsqueeze(0).unsqueeze(0)  # (1,1,obs_dim)
        x = torch.relu(self.input_linear(st))
        out, hnext = self.gru(x, hidden)
        logits = self.actor(out.squeeze(1))  # (1,action_dim)
        probs = torch.softmax(logits, dim=-1)
        dist = torch.distributions.Categorical(probs)
        action = int(dist.sample().cpu().item())
        logp = dist.log_prob(torch.tensor(action, device=self.device))
        value = self.critic(out.squeeze(1)).squeeze(-1)
        entropy = dist.entropy().mean().item()
        return action, logp.squeeze(0), value.squeeze(0), hnext, entropy
...
def sample_physics_from_ranges(cfg):
    ...
    pr = cfg.get("physics_ranges", {})
    if "box2d_gravity" in pr:
        a, b = pr["box2d_gravity"]
        sampled["box2d_gravity"] = float(np.random.uniform(a, b))
    if "wind_power" in pr:
        a, b = pr["wind_power"]
        sampled["wind_power"] = float(np.random.uniform(a, b))
    if "turbulence_power" in pr:
        a, b = pr["turbulence_power"]
        sampled["turbulence_power"] = float(np.random.uniform(a, b))
    return sampled
...
def train(config_path=None):
    ...
    for episode in range(cfg["episodes"]):
        ...
        if episode % 10 == 0:
            ...
            print(f"Ep {episode} Reward {total_reward:.2f} Avg50 {avg50:.2f} Ent {last_ent:.3f}")
